	...
	<div id="container" class="justify-content-center">			
		<div id="body" class="container jumbotron jumbotron-fluid justify-content-center">
			...
			if($_SESSION['role']=='A'){
				...
				echo '<form method="POST" action="'; echo base_url(); echo'index.php/Schedule_CI/enterNew" class="pull-right">
				...
				<thead>
					...
					<?php if($_SESSION['role']=='A'){echo'<button type="submit" class="btn btn-outline-primary btn-info" onClick="return confirm("Are you sure you want to Edit?")"><i class="material-icons">edit</i></button>'; }?>
								</form>
									</div>
									<div class="col-md-2" >
									<form method="POST" action="<?php echo base_url();?>index.php/Schedule_CI/delete/<?php echo $row->SID ?>">
									<?php if($_SESSION['role']=='A'){echo'<button type="submit" class="btn btn-danger" onClick="return confirm("Are you sure you want to delete?")"><i class="material-icons">delete_forever</i></button>';}?>
								</form>
					<?php } ?>
									</div>
